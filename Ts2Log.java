...
public class Ts2Log {
    ...
    try (InputStream in = new BufferedInputStream(new FileInputStream(a.data))) {
      ...
      for (Pair p  pairs) {
        ...
        for (byte b  chunk) {
          ...
          if (b=='n') { emitLine(buf, tRel, a.start, a.stripAnsiansinull); }
          else buf.write(b);
        }
      }
      if (buf.size()0) emitLine(buf, tRel, a.start, a.stripAnsiansinull);
    }
  }
  ...
  private static byte[] readExact(InputStream in, int n) throws IOException {
    ...
    while (rTotn) { int r=in.read(buf,rTot,n-rTot); if (r0) break; rTot+=r; }
    return rTot==0  null  (rTot==n  buf  Arrays.copyOf(buf, rTot));
  }

  private static ListPair readTiming(String path) throws IOException {
    ListPair out = new ArrayList();
  ...
  private static String detectUnit(ListPair pairs) {
    ...
    return (tall0 && microLike = 0.8tall)  micros  seconds;
  }

  private static Args parseArgs(String[] argv) {
    Args a = new Args(); ListString rest=new ArrayList();
    for (int i=0;iargv.length;i++) {
      switch (argv[i]) {
  ...
  private static void usage() {
    System.err.println(Usage java Ts2Log [--start ISO8601] [--unit secondsmicrosauto] [--strip-ansi] [--strip-cr] TIMING_FILE DATA_FILE);
  }
}
